...
class Human(GraspingRobot):
    ...
    def __init__(self, obj, parent=None):
        ...
        self.is_sd = False  # is human sitting down
        self.is_su = False  # is human standing up
    ...
    def look_around(self):
        ...
        myPi = PI / 2
        random_head_l = random.uniform(0.5, 1)
        random_head_r = random.uniform(0.5, 1)
        random_hand_l = random.uniform(0, 0.1)
        random_hand_r = random.uniform(0, 0.1)

        random_direction = random.uniform(0, 2)

        # look left
        N = 5
        if random_direction < 1:
            for i in range(N):  # look left
                target.applyMovement([0, random_head_l * (PI / 2) / N, 0], True)
                hand_l.applyMovement([random_hand_l / N, random_hand_l / N, random_hand_l / N], True)
                time.sleep(0.1)

            for i in range(N):  # back
                target.applyMovement([0, -random_head_l * (PI / 2) / N, 0], True)
                hand_l.applyMovement([-random_hand_l / N, -random_hand_l / N, -random_hand_l / N], True)
                time.sleep(0.1)

            for i in range(N):  # look right
                target.applyMovement([0, -random_head_r * (PI / 2) / N, 0], True)
                hand_r.applyMovement([-random_hand_r / N, -random_hand_r / N, -random_hand_r / N], True)
                time.sleep(0.1)
    ...
    def warn_robot(self):
        ...
        f_speed = (vec2 - vec1)
        # f_speed = [0.6, 0, 0.53]
        # f_speed = [0.6, 0, 0.53] #range[0.5-0.7,0-0.1,0.4-0.6]


        fs_x_r = random.uniform(0.55, 0.65)
        fs_y_r = random.uniform(0.0, 0.1)
        fs_z_r = random.uniform(0.5, 0.6)
        fs_x_l = random.uniform(0.55, 0.65)
        fs_y_l = random.uniform(0.0, 0.1)
        fs_z_l = random.uniform(0.5, 0.6)
        f_speed_r = [fs_x_r, fs_y_r, fs_z_r]  # [0.6-0.8, 0.0-0.1, 0.1-0.3]
        f_speed_l = [fs_x_l, fs_y_l, fs_z_l]  # [0.6-0.8, 0.0-0.1, 0.1-0.3]
        ...
        for i in range(N):
            ...
            hand_r.applyMovement([f_speed_r[0] / N, f_speed_r[1] / N, f_speed_r[2] / N], True)
            hand_l.applyMovement([f_speed_l[0] / N, f_speed_l[1] / N, f_speed_l[2] / N], True)
    ...
    def grasp(self):
        ...
        fs_x_r = random.uniform(0.6, 0.7)
        fs_y_r = random.uniform(0.0, 0.1)
        fs_z_r = random.uniform(0.1, 0.2)
        fs_x_l = random.uniform(0.6, 0.7)
        fs_y_l = random.uniform(0.0, 0.1)
        fs_z_l = random.uniform(0.1, 0.2)
        f_speed_r = [fs_x_r, fs_y_r, fs_z_r]  # [0.6-0.8, 0.0-0.1, 0.1-0.3]
        f_speed_l = [fs_x_l, fs_y_l, fs_z_l]  # [0.6-0.8, 0.0-0.1, 0.1-0.3]
        # print(f_speed_r)
        # print(f_speed_l)
        ...
        if dist < self.MIN_DIST:
            ...
            hand_r.localPosition = [0.25, -0.2, 0.8]
            hand_l.localPosition = [0.25, 0.2, 0.8]
            obj.worldPosition = [hand_r.worldPosition[0] - 0.2, hand_r.worldPosition[1] - 0.3,
                                 hand_r.worldPosition[2] + 0.3]
            N = 5
            # turn right 90 degree
            for i in range(N):
                human.applyRotation([0, 0, math.radians(-90) / N], True)
                obj.worldPosition = [hand_r.worldPosition[0] - 0.3, hand_r.worldPosition[1] - 0.3,
                                     hand_r.worldPosition[2] + 0.3]
                time.sleep(0.1)
